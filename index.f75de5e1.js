...
employeeForm.addEventListener("submit", (formSubmitEvent)=>{
    ...
    const employeeData = {};
    for (const [key, value] of formData.entries())employeeData[key] = value;
    const newRow = document.createElement("tr");
    const validateValue = (key, value)=>{
        if (key === "name" && value.length < 4) return [
            false,
            "Name must be at least 4 characters long"
        ];
        if (key === "age" && (value < 18 || value > 90)) return [
            false,
    ...
    for (const fieldConfig of employeeFormFields){
        const [, error] = validateValue(fieldConfig.name, employeeData[fieldConfig.name]);
        if (error) {
            createNotification("Validation Error", error, "error");
            return;
        }
        const cell = document.createElement("td");
        cell.textContent = transformValue(fieldConfig.name, employeeData[fieldConfig.name]);
...
headers.forEach((header, index)=>{
    ...
    header.addEventListener("click", ()=>{
        ...
        rowsArray.sort((rowA, rowB)=>{
            ...
            if (typeof parsedA === "number" && typeof parsedB === "number") return isAscending ? parsedA - parsedB : parsedB - parsedA;
            return isAscending ? parsedA.toString().localeCompare(parsedB.toString()) : parsedB.toString().localeCompare(parsedA.toString());
        });
        tbody.innerHTML = "";
...
table.addEventListener("click", (clickEvent)=>{
    ...
    if (previouslySelectedRow) previouslySelectedRow.classList.remove("active");
...
table.addEventListener("dblclick", (dblClickEvent)=>{
    ...
    inputField.addEventListener("keydown", (keyEvent)=>{
        if (keyEvent.key === "Enter") saveChanges();
